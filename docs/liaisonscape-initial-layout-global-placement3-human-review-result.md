# LiaisonScape Initial-Layout Global Placement 3 Human Review

Date: 2026-09-12

Status: MIXED; RETUNE REQUIRED; PRODUCT DEFAULT AND RELEASE BLOCKER UNCHANGED

## Review boundary

This checkpoint compares the new `global-placement3` arm with the retained
...
The review used a stable browser surface at approximately 1127 by 1096 CSS
pixels. Human judgment is recorded separately from the machine results in
`liaisonscape-initial-layout-global-placement3-result.md`.

## Six-cell human observations

| Cell | Human judgment | Observation |
| --- | --- | --- |
...
```text
viewport-anisotropic candidate visual quality = MIXED / ACCEPTABLE IN 3 OF 6 CELLS
Titanic JA readability improvement          = CONFIRMED BY HUMAN REVIEW
candidate-only discrete visual regression   = NOT OBSERVED
candidate-specific readability trade-off   = OBSERVED IN LIGHTHOUSE EN/JA, TITANIC EN
full ECR3 visual direction retained         = PARTIAL
Stage-2 bypass                              = RETAIN AS DIAGNOSTIC HYPOTHESIS, NOT ADOPTION
candidate disposition                       = RETUNE
Initial Layout Release blocker              = STILL BLOCKED
Product default/adoption                   = HOLD / UNCHANGED
```

The next bounded study should reduce the anisotropic transform or make it
...
this review does not inherit the ECR3 human ACCEPT.
