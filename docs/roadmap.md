...
| History 2 bounded Stable registration | **IMPLEMENTED / PROFILE REGISTERED / VALIDATOR GREEN / SAMPLE MIGRATION DEFERRED** | [implementation result](history-2-position-circa-stable-profile-implementation-result.md); `history@2.0.0` remains Candidate while `history@2.0.0 / position-circa` is the adopted Stable profile; public H2 sample migration remains a separate checkpoint |
