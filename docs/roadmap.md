...
| NarrativeLine H2 `circa` regression candidate | **NEXT RUNTIME CHECKPOINT / INVESTIGATION REQUIRED** | Human review observed malformed approximate-time presentation and possible loss of time display on other Events after editing a time-bearing Event. Presentation, projection, History mutation/data-preservation, and Dataset-wide/shared-state scope remain unclassified; investigate before visual polish without reopening the accepted bounded scope by assumption |
...
| Cross-App control density / Visual Style / Flatness | **PARTIAL IMPROVEMENT / HUMAN VISUAL ACCEPTANCE NOT YET PASSED** | Recent NarrativeLine and LiaisonScape compact-control/dialog/title-action commits are intermediate evidence, not a final Cross-App baseline. Human review still sees larger ordinary/detail/dialog controls than Hub, remaining radius/language differences, and a `More` treatment mismatch |
| LiaisonScape viewport toolbar | **FOLLOW-UP RETAINED / COMPACTNESS AND FOCUS-TOOLTIP REVIEW PENDING** | Existing [collapse follow-up](liaisonscape-viewport-toolbar-collapse-follow-up.md) and accepted keyboard/focus contract remain authoritative. Human review adds toolbar-density concern and pointer-acquired focus/tooltip evidence; this does not authorize removing keyboard focus |
| NarrativeLine Detail controls | **FOLLOW-UP RETAINED / HUMAN VISUAL REVIEW PENDING** | Event Detail and Entity Detail actions still appear oversized after the bounded density work. Picker/Create may be considered only if later evidence confirms the same ownership; no broader surface is opened here |
...
The current runtime order is therefore: investigate the NarrativeLine H2 `circa`
regression candidate first; then continue the bounded Cross-App density/Flatness,
LiaisonScape viewport-toolbar, and NarrativeLine Detail-control follow-ups. This
does not reopen the accepted H2-POSITION-CIRCA scope or promote any deferred
History 2 authoring surface.
